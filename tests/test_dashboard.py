...
class DashboardTests(unittest.TestCase):
    ...
    def test_dashboard_keeps_poster_listing_work_in_poster_section(self):
        ...
        self.assertIn("Mug performance · <span data-mug-period-label>Past 24 hours</span>", response.text)
        self.assertIn('class="is-active" data-traffic-period="24h"', response.text)
        self.assertIn("let activePeriod = '24h'", response.text)
        self.assertIn("metric(item,'mug_views')", response.text)
        self.assertIn("Most viewed", response.text)
        self.assertIn("Least viewed", response.text)
        self.assertIn('data-mug-sort', response.text)
        self.assertIn("Past 24 hours", response.text)
        self.assertIn('data-analytics-exclusion', response.text)
        self.assertIn('shangoolios-analytics-exclusion-confirmed', response.text)
        self.assertIn("Daily storefront funnel", response.text)
        self.assertIn("Ad spend", response.text)
        self.assertIn("Profit (est.)", response.text)
        self.assertIn("commerceByDate", response.text)
        self.assertNotIn("data-commerce-daily", response.text)
        ...
        self.assertNotIn("data-commerce-performance", response.text)
        self.assertIn("Sync sales &amp; ads", response.text)
        self.assertIn('href="/pinterest-ads/connect"', response.text)
        self.assertIn("Collection", response.text)
        self.assertIn("collection_name", response.text)
