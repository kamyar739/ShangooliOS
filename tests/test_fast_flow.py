...
class FastFlowTests(unittest.TestCase):
    ...
    def test_fast_flow_page_is_an_independent_navigation_destination(self):
        ...
        self.assertIn("Add the collection package from ChatGPT", response.text)
        self.assertIn("Collection information", response.text)
        self.assertIn("Final review", response.text)
        self.assertIn("Create Collection &amp; Import Artworks", response.text)
