...
def build_production_summary(
    ...
    mockup_status = []
    for default_position, (slot_key, label, description) in enumerate(
        MOCKUP_SLOTS, start=1
    ):
        assignment = assignment_map.get(f"mockup:{slot_key}")
        exists = bool(
            assignment
            and assigned_file_exists(artwork, assignment["relative_path"])
        )
        mockup_status.append(
            {
                "slot_key": slot_key,
                "label": label,
    ...
    missing_mockups = [
        record["label"] for record in mockup_status if not record["exists"]
    ]
    ...
    return {
        ...
        "missing_mockups": missing_mockups,
        "mockups_complete": not missing_mockups,
